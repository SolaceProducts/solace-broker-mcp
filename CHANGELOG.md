...
- Release artifacts now carry build provenance attestations. The release workflow attaches a GitHub artifact attestation to each of the four binary archives and to the multi-arch container image, so a consumer can prove an artifact was produced by this repository's release pipeline at a specific commit rather than rebuilt or substituted elsewhere — a guarantee the existing `checksums-sha256.txt` cannot give, since it only proves a file matches the checksum list published beside it. Each archive is attested in the matrix job that built it, so the provenance names the real builder. The image attestation is pushed to the registry alongside the image, so verification does not require the GitHub API. Verify with the GitHub CLI: `gh attestation verify <archive> --repo SolaceProducts/solace-broker-mcp` for an archive, and `gh attestation verify oci://ghcr.io/solaceproducts/solace-broker-mcp:<tag> --repo SolaceProducts/solace-broker-mcp` for the image. Signed *git tags* remain **[Planned]** — a separate mechanism, see `RELEASING.md`. Tracked under SOL-152543.
