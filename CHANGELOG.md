...
- Tools declaring `idempotent: false` are no longer retried on failures where the broker may already have carried out the request, so a single `delete-queue-messages` or `disconnect-client` call can no longer reach the broker more than once. The SEMP retry policy inferred replay safety from the HTTP method, excluding only POST and PATCH on the grounds that RFC 9110 defines PUT and DELETE as idempotent. That reasoning holds for the SEMPv2 `config/` namespace, where a repeated resource replacement converges on the same state, and breaks for the `action/` namespace, which routes non-idempotent destructive RPC over PUT. RFC 9110 idempotency constrains a request's *final state*, not the set of side effects produced along the way: `doMsgVpnQueueDeleteMsgs` accepts no message-ID range, no timestamp watermark, and no sequence bound — it is specified as "Delete all spooled messages from the Queue" — so a replay purged whatever producers had spooled since the caller's request while the caller was told the call had failed. Any transport-level error after the broker received the request (a TCP reset, a broker restart mid-request, a load-balancer idle drop) was enough to trigger it, and that path carried no sub-cap, so it could run to the full `semp.retries` budget (11 requests at the shipped default of 10). Tool definitions already declared `idempotent: false`, but the annotation reached only the MCP schema, never the retry layer; the composite executor now carries that declaration into the SEMP retry policy via a new caller-declared marker (the inverse of the existing retry-safe marker used by read-only SEMPv1 commands). Marked requests still recover from `401 Unauthorized` — an authentication rejection is issued before the broker acts on the request, so re-authenticating cannot duplicate a side effect, and a token expiring mid-request must not become a hard failure. Retries are otherwise suppressed for those tools, including on `429`/`503` and other `5xx`: a `504` in particular can fire while the broker is still working, making a replay the same hazard as a broken connection. Behaviour is unchanged for every other tool — SEMPv2 `GET`, retry-safe SEMPv1 `<show>` commands, and the `config/` `PUT`/`DELETE` write tools all keep their existing retry resilience — and unchanged for the two `action/` tools that declare `idempotent: true` (`clear-queue-stats`, `clear-client-stats`). Only `delete-queue-messages` and `disconnect-client` are affected, both of which require `enable_write_tools: true`. Operators running those tools against a broker that drops the connection mid-call will now see the call fail once rather than silently succeed several times. Tracked under SOL-152400.
