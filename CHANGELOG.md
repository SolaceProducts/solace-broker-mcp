...
- New MCP tool `describe-schema` for retrieving the SEMPv2 schema slice of a given operation's request-body definition, so an agent planning a create or update can enumerate every configurable attribute (with types, defaults, enum values, and writability flags) before invoking the write tool. Read-only and broker-independent — registered standalone like `list-brokers`, with no `broker` parameter and no RBAC surface, since the response is derived from the embedded OpenAPI specs and reveals nothing broker-specific. Takes an `operation` identifier in `<specType>/<operationId>` form (e.g. `config/createMsgVpnQueue`) and an optional `view`: `trimmed` (default) returns a compact per-attribute list surfacing SEMP's `x-*` writability extensions as `writableOnCreate`/`writableOnUpdate`/`requiredForCreate`/`identifying`/`writeOnly`/`sensitive`/`deprecated`/`requiresDisable`; `raw` returns the OpenAPI definition verbatim. Tracked under SOL-152603.
...
- Each of the eight `create-*`/`update-*` write tools (message-VPN, queue, topic endpoint, RDP) now carries a one-line pointer to the new `describe-schema` tool with the SEMPv2 operation identifier the caller passes (e.g. `config/createMsgVpnQueue`), so an agent that needs the full attribute list beyond the ones inlined in the description knows where to find it. The `BEFORE INVOKING THIS TOOL` confirmation block was also lifted to the top of every write tool's description so the confirmation requirement is the first thing an agent reads. Description text only; no parameter or output-shape change. Tracked under SOL-152603.
