...
- The breaker's state-change WARN now carries a `consecutive_failures` attribute, so an operator can tell which rule (consecutive vs. rate) opened the breaker from the log line alone. Tracked under SOL-152286.
