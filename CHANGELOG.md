...
- New top-level `tool_authorization:` configuration block for per-tool authorization based on the caller's group or role memberships as carried by a configurable OIDC claim. Schema-and-validation only in this release — the runtime is gated behind the `ENABLE_TOOL_AUTHORIZATION` environment variable, and with the flag unset (the default) the block is parsed but otherwise inert: no defaults are applied, no validation fires, and existing deployments are unaffected. The block holds `enabled`, `groups_claim_name` (name of the OIDC claim carrying the memberships; defaults to `"groups"`), and `access_level_groups` (a map from group name to the list of MCP tool names that group grants). When the flag is set truthy, the block's defaults and validation activate. `list-brokers` is structurally exempt — every authenticated caller can invoke it regardless of the policy, so a caller can always discover configured broker aliases before invoking any other tool; listing it under a group in `access_level_groups` is inert. Authorization decisions live in a new `internal/authz` package; its `Decision` type implements `slog.LogValuer` to emit only an `allowed` flag and a matched-group count, never the group names themselves, so the sanctioned logging path cannot leak group membership. Tracked under SOL-151875 and SOL-151907.
...
- Tool authorization is now unconditionally active under `mcp_client_auth.mode: oauth`. The `ENABLE_TOOL_AUTHORIZATION` environment variable that previously gated the feature has been removed, and the `tool_authorization` block's `enabled` field is now required in oauth mode — it must be set explicitly to `true` (opt in, with `access_level_groups` populated) or `false` (opt out). Omitting the block, or omitting `enabled` from it, is a startup error. Every oauth deployment therefore makes a deliberate choice on tool-level access control; the previous silent default of "off" is gone. Tracked under SOL-152293.
### Removed
