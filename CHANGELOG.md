...
- Removed the dangling "See the SEMP `<Type>` schema for the full list." pointer from the `create-vpn`, `update-vpn`, `create-queue`, `update-queue`, `create-topic-endpoint`, `update-topic-endpoint`, `create-rdp`, and `update-rdp` write-tool descriptions. The MCP server does not expose the SEMP schema, so the sentence sent agents chasing a resource they cannot reach; the attribute examples already inline in each description remain. Tracked under SOL-152203.
