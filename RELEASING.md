...
3. Nothing to do here manually — see **Third-party licence inventory** below.

### Third-party licence inventory

**Source of truth: the Go module graph, not FOSSA — SOL-153190.** `THIRD_PARTY_LICENSES.md` is generated from `go-licenses` against `go list -deps ./cmd/server`, the same closure `.github/scripts/licenses-check.sh` verifies. That is the whole reason to pick it: a generator working from this closure needs no reconciliation with what the checker expects, because its output IS what the checker expects. Reconciling against FOSSA's own dependency resolution — the instruction this replaces — was a standing source of drift, since FOSSA's resolution is not the build's resolution, and a FOSSA-shaped file could fail the very gate that guards the release.

FOSSA is not dropped by this decision. It remains the authoritative *policy* gate — `Guardian scan gate`, required on every pull request and re-run at the tag — for licence and vulnerability *policy*. This document is the human-readable *inventory*, a different obligation the Solace public-repository Legal checklist tracks separately. Nothing here represents or substitutes for a FOSSA verdict.

**Automatic regeneration is tracked separately, under SOL-152956** (which also covers `THIRD_PARTY_BUILD_TEST.md`, out of scope here) — a workflow that regenerates the inventory from this same closure and pushes the fix onto a pull request's own branch when `Third-party licenses current` fails on ordinary drift. This document will name that mechanism's actual script/job once it lands; until then, this stays a manual, hand-edit step: `go run github.com/google/go-licenses@v1.6.0 csv ./cmd/server` supplies the module, version, and licence data — it prints to stdout, it does not write the file — and a human reconciles that into `THIRD_PARTY_LICENSES.md`'s table format, then confirms with `.github/scripts/licenses-check.sh`.
