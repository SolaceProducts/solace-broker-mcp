...
func newConfigStore() *configStore {
	return &configStore{perPort: make(map[int]*atomic.Pointer[portOverride])}
}
...
// set replaces the override for a port. The map itself is grown at
// registration time (once per POST, not on the request path), and the
// pointer swap is the atomic operation that requests race against.
func (s *configStore) set(port int, o portOverride) {
	p, ok := s.perPort[port]
	if !ok {
		p = &atomic.Pointer[portOverride]{}
		s.perPort[port] = p
	}
	p.Store(&o)
}
