...
test_error_translation() {
    ...
    assert_json_field "$resp" ".result.structuredContent.outcome" "already_exists" \
        "error-xlate: duplicate create reports outcome=already_exists" || return 1
    assert_json_field "$resp" ".result.structuredContent.changed" "false" \
        "error-xlate: duplicate create reports changed=false" || return 1
    assert_json_field "$resp" ".result.structuredContent.attributes_verified" "false" \
        "error-xlate: duplicate create reports attributes_verified=false (existence only, not a config match)" || return 1
