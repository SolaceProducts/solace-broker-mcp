...
| 8 | `list-brokers` is structurally exempt | A caller the policy grants nothing can still call `list-brokers`, and **no** audit record is emitted — `RegisterListBrokers` takes no policy, so the gate is never wrapped around it |
