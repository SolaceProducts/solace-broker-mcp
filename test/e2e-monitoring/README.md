...
| F4       | Sustained traffic        | Publisher targets 100 msg/s, 256-byte payload, persistent; broker observes ~92 msg/s sustained after ~5 s settle. **Verification:** after 25 s of fixture runtime, peak `rxMsgRate ≥ 80` and peak `txMsgRate ≥ 40` over 5 polls / ~5 s (tx is lower/noisier than rx). | background broker-driver           | `get-message-rates`                                                                |
