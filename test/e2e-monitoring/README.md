...
| F3       | Connected client         | One long-lived persistent receiver per broker on default VPN with deterministic `clientName` and ≥1 named topic subscription. **Verification:** client appears in `list-clients` and reports the expected subscription. Also doubles as the regression fixture for SOL-153071: the `real-clients` fan-out probe's old `count=1` scan only ever reached a reserved (`#`-prefixed) client and never scanned far enough to see a real client behind it, so every enabled+up VPN was miscounted as zero-connection. Once F8's bridges and F9/F10's Kafka Receivers/Senders are also active on this same default VPN, F3's real client sits behind *multiple* reserved clients — lab-measured: `msgVpns/default/clients` returns zero real clients at `count=1` or `count=2`, partial at `count=5`, all of them at `count>=10` — reproducing the production bug shape (AC4) without a dedicated VPN. An earlier design provisioned a separate `test-vpn-real-client` VPN for this, but this broker image caps message-VPN count at 3 total (including `default`), already exhausted by F1's `test-vpn`/`test-vpn-empty` — reusing the default VPN avoids that limit. | background broker-driver           | `list-clients`, `get-client-details`, `list-client-subscriptions`, `list-vpns` (`zeroConnectionCount` false-positive regression coverage) |
