...
"no", so renaming or dropping a standing fixture here breaks a scenario there.
...
| F1       | Multi-VPN                | Two additional non-default VPNs on each broker: `test-vpn` with `enabled=false` (feeds `list-vpns.disabledCount`), and `test-vpn-empty` with `enabled=true` and no client-user / ACL / queue setup — feeds `list-vpns.zeroConnectionCount`, which the handler derives from the `real-clients` fan-out probe (`clientUsername != #*`), not from the connection count. This fixture is also the recorded observation that an idle enabled+up VPN on these brokers reports `msgVpnConnections==1`, the reserved `#client`: SOL-154166's zero-connection probe skip therefore does not fire here, and the probe still runs. | one-shot SEMP                      | `list-vpns` (`disabledCount`, `zeroConnectionCount` aggregation coverage), `get-vpn-status` (enabled + disabled state coverage) |
...
| F3       | Connected client         | One long-lived persistent receiver per broker on default VPN with deterministic `clientName` and ≥1 named topic subscription. **Verification:** client appears in `list-clients` and reports the expected subscription. Also doubles as the regression fixture for SOL-153071: SEMP applies the `real-clients` probe's `where` filter *after* its `count` cut, so a one-object scan window could be consumed by a reserved (`#`-prefixed) client and return nothing even though a real client was present elsewhere in the VPN. Reserved clients are not sorted ahead of real ones — measured order on this fixture combination (F3's real client plus F8's bridges and F9/F10's Kafka Receivers/Senders, all on the default VPN) put reserved names on both sides of the real one — so the fix scans exhaustively (`forceFullPage=true`) rather than raising a fixed `count` bound. The e2e regression check derives its prediction from the scan position actually observed on each run rather than assuming reserved-first. An earlier design provisioned a separate `test-vpn-real-client` VPN for this, but this broker image caps message-VPN count at 3 total (including `default`), already exhausted by F1's `test-vpn`/`test-vpn-empty` — reusing the default VPN avoids that limit. | background broker-driver           | `list-clients`, `get-client-details`, `list-client-subscriptions`, `list-vpns` (`zeroConnectionCount` false-positive regression coverage) |
