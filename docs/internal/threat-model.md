...
| A maintainer clears a failed `DCO` via the App's override button (Repudiation) | None today. `dco2` defaults `allowOverrideAction: true` and this repo has no `.github/dco.yml`; upstream notes it is not a security boundary, since write access implies config access. Closable by committing `allowOverrideAction: false` | **No mitigation — accepted risk** |
