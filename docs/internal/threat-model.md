...
| Same, but the check is bypassable | **Confirmed live**: `Third-party licenses current` is documented as required (`ADMIN_SETUP.md:208-209`) but is **not** in the actual required-status-checks list on `main` (`gh api .../rulesets/13942241` → only `lint`, `build`, `Guardian scan gate`, `DCO sign-off`, `DCO check self-test`) | **No mitigation — accepted risk, contradicts documented intent. Worth a follow-up ticket to either add it to the ruleset or correct the doc.** |
...
| Unattributable or forged commit authorship (Repudiation) | `DCO sign-off` and `DCO check self-test` are both live-required checks (`gh api .../rulesets/13942241`), the base-ref check runs via `pull_request_target` so a PR can't neuter its own gate, and the Dependabot exemption is keyed on GitHub's own bot-login field, not a spoofable commit author | Mitigated |
