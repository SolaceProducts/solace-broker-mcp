...
| Token leak via cache logging (Info Disclosure) | `CachedCredential`/`GetResult` `LogValue()` explicitly omit the token value (`internal/oauth/cache/cache.go:LogValue`) | Mitigated |
