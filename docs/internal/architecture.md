...
| **Audit log** | Implemented | Record emission ships (`internal/observability/audit/emit.go:Emit`, `EmitDrop`), gated by `tools.WithAuditLog` / `audit.Enabled`. Destructive `operation` records plus the other wired event types in `docs/observability.md`. Opt-in, default OFF (`OBS_AUDIT_LOG_ENABLED`). Best-effort; off means inert, not degraded. |
