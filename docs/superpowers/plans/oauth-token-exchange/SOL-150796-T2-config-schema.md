...
**Status:** schema landed in PR #109; runtime ships in follow-up sub-tickets.
...
**Scope of this doc:** T2-specific decisions — schema shape, field naming, validator rules, the V1 OAuth-not-supported guard, the Hop 1 / Hop 2 alignment invariant, the cross-IdP audit, and the naming principle that emerged from the field-rename sweep.

**Why split out of the umbrella SOL-150070 decisions doc:** the umbrella file was approaching 1100 lines while we were still inside T2. Splitting per sub-ticket keeps each file readable and lets future sub-tickets (T3, T4, …) live in their own siblings under [`oauth-token-exchange/`](.) instead of compounding the umbrella file.
