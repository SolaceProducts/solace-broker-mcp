...
.PHONY: test-cover
test-cover: ## Run unit tests with coverage; fails if aggregate coverage drops below 80% (matches CI; see docs/internal/unit-test-coverage.md)
	go test -race -v -coverprofile=coverage.out ./...
	@total=$$(go tool cover -func=coverage.out | tail -1 | awk '{print $$3}' | tr -d '%'); \
	echo "Total coverage: $$total%"; \
	awk -v t="$$total" 'BEGIN { if (t+0 < 80.0) { print "FAIL: coverage " t "% is below the 80% floor"; exit 1 } else { print "OK: coverage " t "% meets the 80% floor" } }'
