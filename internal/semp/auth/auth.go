...
// one new implementation plus one new case in NewAuthenticator.
//
...
//
// exchanger is required for oauth mode and ignored for other modes.
...
func NewAuthenticator(cfg config.AuthConfig, exchanger *tokenexchange.Exchanger, jar CookieJarClearer) (Authenticator, error) {
	switch cfg.Mode {
	...
	case config.AuthModeOAuth:
		return NewOAuthAuthenticator(exchanger, cfg.Audience, cfg.Scopes), nil
	default:
