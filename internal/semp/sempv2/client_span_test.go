...
// span has been started — standing in for a programmer error anywhere between
// tracer.Start and the deferred span close.
type panickingAuthenticator struct{}

func (panickingAuthenticator) AddAuth(context.Context, *http.Request) error {
	panic("deliberate panic below sempv2 Execute")
}
...
// caught in review, and the composite executor's equivalent test.
func TestExecute_PanicMarksSpanAsErrorAndRepanics(t *testing.T) {
	...
	func() {
		...
		_, _ = client.Execute(context.Background(),
			&sempv2.Operation{ID: "getMsgVpn", Method: http.MethodGet, Path: "/SEMP/v2/monitor/msgVpns/default"},
			map[string]any{})
	}()
	...
	if attrs["semp.version"] != "v2" {
		t.Errorf("semp.version = %q, want v2 (the attribute set must still be recorded on the panic path)", attrs["semp.version"])
	}
