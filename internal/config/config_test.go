...
// -----------------------------------------------------------------------------
// Hop 2 OAuth config tests (SOL-150796)
// -----------------------------------------------------------------------------

// TestLoadConfig_NoBrokerOAuthBlock_BackwardsCompat verifies that an existing
// YAML configuration that predates the broker_oauth block continues to load
// cleanly. No broker uses oauth mode, no broker_oauth block is present, and
// the server should start without warnings or errors related to the new block.
...
func TestValidateHop1Hop2Alignment_Direct(t *testing.T) {
	...
	t.Run("hop1 static + hop2 oauth — invariant violated", func(t *testing.T) {
		...
		msg := err.Error()
		// Operator-facing error must name (a) the offending Hop 1 mode and
		// (b) why Hop 2 cannot work without Hop 1 oauth.
		for _, want := range []string{`"static"`, "1 broker", "auth.mode: oauth", "subject_token", "mcp_client_auth.mode: oauth"} {
			if !strings.Contains(msg, want) {
	...
	t.Run("hop1 static + 3 hop2 brokers — plural noun", func(t *testing.T) {
		cfg := &ServerConfig{
			MCPClientAuth: MCPClientAuthConfig{Mode: AuthModeStatic, DevToken: "t"},
...
// TestLoadConfig_Hop1Hop2AlignmentError verifies that, even in V1 where the
// not-yet-supported guard fires for every oauth broker, the alignment
// validation error still lands in the joined error blob. The invariant is
// thereby enforced today; only the secondary banner is suppressed.
...
`
	...
	if strings.Contains(out, "Hop 2 token exchange (RFC 8693)") || strings.Contains(out, "requires OAuth client") {
		t.Errorf("Hop1/Hop2 alignment banner should be suppressed while V1 guard is active, got:\n%s", out)
	}
}

// TestBrokerOAuthConfig_LogValue verifies that the secret nested inside the
// client_auth sub-block is excluded from log output via the LogValuer
// implementations on BrokerOAuthConfig, BrokerClientAuth, and ClientSecretAuth.
