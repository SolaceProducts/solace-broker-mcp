...
func TestNew_NilResourceIsRejected(t *testing.T) {
	...
	cfg := config.ObservabilityConfig{TracingEnabled: true, MetricsScrapeEnabled: true}
...
func TestNew_NonPositiveSelfStatsInterval_DoesNotPanic(t *testing.T) {
	...
	cfg := config.ObservabilityConfig{TracingEnabled: true, MetricsScrapeEnabled: false, OTelSelfStatsIntervalS: 0}
	p, err := New(cfg, nil, sdkresource.Default())
	if err != nil {
		t.Fatalf("New() error = %v, want nil", err)
	}
...
func TestProvider_Shutdown_StopsSelfStatsEmitter(t *testing.T) {
	...
	cfg := config.ObservabilityConfig{TracingEnabled: true, MetricsScrapeEnabled: false, OTelSelfStatsIntervalS: 1}
