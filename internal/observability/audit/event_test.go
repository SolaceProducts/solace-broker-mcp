...
// applicabilityName renders one applicability value by name — independent of
// the underlying uint8, so a reordering of the fieldForbidden/fieldOptional/
// fieldRequired/fieldOnErrorOnly iota block cannot pass the fingerprint below
// by accident.
func applicabilityName(a applicability) string {
	switch a {
	case fieldForbidden:
		return "forbidden"
	case fieldOptional:
...
// renderApplicabilityFingerprint deterministically renders every row of
// applicabilityByType (sorted by audit_event_type) plus the schema version
// they are pinned against, one field per column in a fixed order. Comparing
// this — not the Go struct's memory layout — is what makes the fingerprint a
// content check: unexported field names or column order changing without any
// actual policy change would not move it.
func renderApplicabilityFingerprint() string {
	types := make([]string, 0, len(applicabilityByType))
	...
	for _, t := range types {
		r := applicabilityByType[EventType(t)]
		fmt.Fprintf(&b, "%s: outcome=%s error_type=%s reason=%s tool=%s arguments_hash=%s broker=%s timing=%s identity=%s dropped_event_type=%s\n",
			t,
			applicabilityName(r.outcome),
			applicabilityName(r.errorType),
			applicabilityName(r.reason),
			applicabilityName(r.tool),
			applicabilityName(r.argumentsHash),
			applicabilityName(r.broker),
			applicabilityName(r.timing),
			applicabilityName(r.identity),
			applicabilityName(r.droppedEventType),
		)
	}
	return b.String()
}

// wantApplicabilityFingerprint is a hand-maintained snapshot of
// applicabilityByType, keyed to the schema.AuditSchemaVersion that shipped
// it. Neither side is regenerated by a flag on purpose (unlike
// internal/observability/metrics' `-update` golden): a mismatch here must
// force a human decision, not a rerun.
//
// To update: change applicabilityByType, bump schema.AuditSchemaVersion in
// the same commit — additive (a new audit_event_type, or a field moving from
// forbidden to optional/required) bumps the minor version; anything that
// narrows an existing field (required to forbidden, or a value leaving
// allowedOutcomes/reasonVocab) is a major bump — then run
...
authz_denied: outcome=forbidden error_type=forbidden reason=required tool=required arguments_hash=forbidden broker=forbidden timing=forbidden identity=optional dropped_event_type=forbidden
broker_auth_retry: outcome=required error_type=forbidden reason=forbidden tool=forbidden arguments_hash=forbidden broker=required timing=optional identity=optional dropped_event_type=forbidden
broker_authz_denied: outcome=forbidden error_type=forbidden reason=required tool=required arguments_hash=forbidden broker=required timing=forbidden identity=optional dropped_event_type=forbidden
operation: outcome=required error_type=on_error_only reason=forbidden tool=required arguments_hash=required broker=required timing=required identity=optional dropped_event_type=forbidden
`

// TestApplicabilityByType_isVersioned pins schema.AuditSchemaVersion against
// the shape of applicabilityByType, together, so that a change to the table
// without a version bump — or a version bump with no table change — fails
// here instead of shipping as a silent breaking change to a schema whose
// whole thesis is that a customer's SIEM query pins against
// audit_schema_version. See wantApplicabilityFingerprint's own doc comment
// for how to update it.
func TestApplicabilityByType_isVersioned(t *testing.T) {
	t.Parallel()
	got := renderApplicabilityFingerprint()
	if got != wantApplicabilityFingerprint {
		t.Errorf("applicabilityByType and/or schema.AuditSchemaVersion changed without the other "+
			"being updated to match.\ngot:\n%s\nwant:\n%s\n"+
			"If this is a deliberate change, bump schema.AuditSchemaVersion (if not already done) "+
			"and update wantApplicabilityFingerprint to match — see its doc comment for additive "+
			"vs breaking.", got, wantApplicabilityFingerprint)
	}
}
