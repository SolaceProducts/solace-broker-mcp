...
// TestIntegration_QueueClearStats runs the non-destructive `clearStats` action
// against a real broker. Only the non-destructive action is tested live;
// `deleteMsgs` is never executed by automated tests to avoid data loss.
//
// Run with:
//
//	go test -tags=integration -count=1 -run TestIntegration_QueueClearStats \
//	  ./internal/tools/sempv2/queueaction/
//
...
func TestIntegration_QueueClearStats(t *testing.T) {
