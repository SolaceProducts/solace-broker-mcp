...
// TestDisconnect_Metadata pins the destructive tool's surface.
func TestDisconnect_Metadata(t *testing.T) {
	meta := NewDisconnectHandler().Metadata()

	if meta.Name != "disconnect-client" {
		t.Errorf("Name = %q, want %q", meta.Name, "disconnect-client")
	}
	...
	if meta.Annotations.Idempotent {
		t.Error("Idempotent must be false for disconnect-client")
	}
}

// TestClearStats_Metadata pins the non-destructive tool's surface.
func TestClearStats_Metadata(t *testing.T) {
	meta := NewClearStatsHandler().Metadata()

	if meta.Name != "clear-client-stats" {
		t.Errorf("Name = %q, want %q", meta.Name, "clear-client-stats")
	}
	if meta.Annotations.ReadOnly {
		t.Error("ReadOnly must be false (clear-client-stats mutates broker state)")
	}
	if meta.Annotations.Destructive == nil || *meta.Annotations.Destructive {
		t.Errorf("Destructive = %v, want explicit false", meta.Annotations.Destructive)
	}
	if !meta.Annotations.Idempotent {
		t.Error("Idempotent should be true for clear-client-stats")
	}
	if strings.Contains(meta.Description, "BEFORE INVOKING THIS TOOL") {
		t.Error("clear-client-stats description must not carry the confirmation instruction")
	}
	if !strings.Contains(meta.Description, "Non-destructive") {
		t.Error("clear-client-stats description should state it is non-destructive")
	}
}

// TestDisconnect_Success verifies the destructive path: right PUT operation,
// right args, strict success envelope.
func TestDisconnect_Success(t *testing.T) {
	stub := &fixtureClient{}
	tc := &tools.ToolContext{SEMPv2Client: stub}

	result, err := NewDisconnectHandler().Handle(context.Background(), tc, map[string]any{
		"msgVpnName": "default",
	...
	if stub.lastOp.ID != "doMsgVpnClientDisconnect" {
		t.Errorf("op.ID = %q, want doMsgVpnClientDisconnect", stub.lastOp.ID)
	}
	for k, want := range map[string]any{"status": "ok", "msgVpnName": "default", "clientName": "SlowConsumer_12345"} {
...
// TestClearStats_Success verifies the non-destructive path.
func TestClearStats_Success(t *testing.T) {
	...
	if stub.lastOp.ID != "doMsgVpnClientClearStats" {
		t.Errorf("op.ID = %q, want doMsgVpnClientClearStats", stub.lastOp.ID)
	}
}

// TestHandle_EmptyParams_Rejected is the defense-in-depth check.
func TestHandle_EmptyParams_Rejected(t *testing.T) {
	cases := []struct {
		name, vpn, client, wantErr string
	}{
		{"empty vpn", "", "C1", "disconnect-client: msgVpnName is required"},
		{"empty client", "default", "", "disconnect-client: clientName is required"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			stub := &fixtureClient{}
			_, err := NewDisconnectHandler().Handle(context.Background(),
				&tools.ToolContext{SEMPv2Client: stub}, map[string]any{"msgVpnName": tc.vpn, "clientName": tc.client})
...
func TestHandle_SEMPError_Passthrough(t *testing.T) {
	...
	_, err := NewDisconnectHandler().Handle(context.Background(),
		&tools.ToolContext{SEMPv2Client: stub}, map[string]any{"msgVpnName": "default", "clientName": "C1"})
	if err == nil {
