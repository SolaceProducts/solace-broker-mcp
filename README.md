...
A Model Context Protocol (MCP) server for Solace event brokers, built with Go using the official [MCP Go SDK](https://github.com/modelcontextprotocol/go-sdk).
...
| Queues | `list-queues`, `get-queue-metrics`, `list-queue-subscriptions` | List queues with cumulative spooled count and throughput; drill into a single queue for authoritative current depth, spool, and rates; list a queue's topic subscriptions |
...
- [Observability](docs/observability.md) — enable metrics, audit records, traces, correlation IDs, and saturation logs; includes the live metric and audit schema
...
> **Note:** If the repository is private, authenticate with the GitHub Container Registry (GHCR) before pulling:
> ```bash
...
The server requires a YAML configuration file (event broker definitions) and credentials (via `.env` file or environment variables). Environment variables override file paths and port settings.
...
|---|---|---|
| Configuration file path | `CONFIG_FILE` | `broker-config.yaml` in current directory |
| Credentials file path | `ENV_FILE` | `.env` next to configuration file |
...
Environment variables take precedence over YAML settings, which take precedence over defaults.
...
When both are configured, the server starts with HTTPS. When neither is configured, the server uses plain HTTP. Providing only one is a startup error. The certificate must carry at least one DNS or IP SAN. See [Configuration](docs/configuration.md) for why, and for the container health check consequence.
...
We welcome contributions! See our [Contributing Guidelines](.github/CONTRIBUTING.md) for details on:
...
Read our [Code of Conduct](.github/CODE_OF_CONDUCT.md) before participating.
