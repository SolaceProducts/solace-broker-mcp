...
- `auth.mode` — `basic`, `bearer`, or `oauth` (the following examples use basic authentication; for bearer token authentication, set `auth.mode: bearer` and provide `auth.token` instead; for OAuth token exchange, see [Step 2b: Configure event broker OAuth (Hop 2)](docs/authentication.md#step-2b-configure-broker-oauth-hop-2))
