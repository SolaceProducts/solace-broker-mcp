...
Nothing here constrains how we license or distribute the product. All components
are permissive (MIT, BSD-3-Clause, Apache-2.0), and none of them ships. No
strong-copyleft licence (GPL, LGPL, AGPL, EPL, CDDL) appears anywhere in the
build or test path.
...
- Every external Go module in the two submodules' `go list -deps -test` closure
  has a row here, at the version actually in use, and every documented module is
  still in a closure.
- Every `uses:` reference in `.github/workflows/` has a row.
- Every container image referenced in `Dockerfile` or the compose files has a row.
- The reverse direction in all three cases, so a row for something we removed
  fails too.

What it does not enforce: licence *names*. Determining that a module's licence
changed between versions needs the network and a warm module cache. The same
limitation applies to `licenses-check.sh` and is documented there for the same
reason. When you bump a version, open the licence link.
