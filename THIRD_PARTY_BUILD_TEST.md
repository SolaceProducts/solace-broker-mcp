...
### Root Module (test-only)

Until 2026-08-28, `go list -deps -test ./...` on the root module resolved to
exactly the same external module set as `go list -deps ./cmd/server` — see
[Root Module History](#root-module-history) below for that finding, which
held for over two years. SOL-152417 (Story 45, the OTel Prometheus exporter
fidelity spike) is the first thing to break it: `exporter_fidelity_test.go`
imports the OTel metrics/traces SDK and `client_golang` **only** from a
`_test.go` file, so these packages never appear in `go list -deps ./cmd/server`
(never linked into the shipped binary) but do appear in
`go list -deps -test ./...`.

**`.github/scripts/build-test-licenses-check.sh` does not catch this** — its
`discover` loop only walks `find test -name go.mod` (the `test/e2e-*`
submodules), never the root module itself, so a test-only root-module
dependency is a real blind spot, not a gate that fails closed. This section is
maintained by hand for that reason; it is not enforced by CI the way the
tables below it are.

**The list below is deliberately not a `| \`component\` | version |` table.**
The script's own reverse-direction check (§5: "a row for something nothing
uses any more fails") treats *any* line matching that shape as a documented
component that must appear in its `in_use` set — and `in_use` is built only
from checks 1–4 (`test/e2e-*` go.mod closures, npm lockfiles under `test/`,
GitHub Actions, container images), which by design never includes the root
module. Using the real table markup here was tried first and confirmed to
fail closed: every row below was flagged `is listed but nothing in the
repository uses it any more`, for the packages this very entry exists to
document. A bullet list keeps this section honest and human-readable without
tripping a check that structurally cannot see the root module.

**Not a release-inventory change.** `THIRD_PARTY_LICENSES.md` (the shipped-binary
list) is untouched — none of the following is linked into `cmd/server`. That
changes only once Story 14 makes these real production dependencies, at which
point this section is removed and the rows move to `THIRD_PARTY_LICENSES.md`
instead.

Four packages were added directly (`go get`, pinned 2026-08-28, current stable
at the time): `go.opentelemetry.io/otel`, `go.opentelemetry.io/otel/sdk`,
`go.opentelemetry.io/otel/exporters/prometheus`, `github.com/prometheus/client_golang`.
Their transitive closure pulls in the rest of the table below. Every license was
read directly from the module's own `LICENSE` file in the local module cache,
not inferred from a package name — the OTel and Prometheus rows are Apache-2.0
as expected, but the closure also carries MIT and BSD-3-Clause rows the "four
packages, all Apache-2.0" framing in the spike's working plan did not
anticipate. All are permissive; none changes the Verdict above.
...
- `go.opentelemetry.io/otel/sdk` v1.46.0 — Apache-2.0 — [license](https://github.com/open-telemetry/opentelemetry-go/blob/sdk/v1.46.0/sdk/LICENSE)
- `go.opentelemetry.io/otel/metric` v1.46.0 — Apache-2.0 — [license](https://github.com/open-telemetry/opentelemetry-go/blob/metric/v1.46.0/metric/LICENSE)
- `go.opentelemetry.io/otel/trace` v1.46.0 — Apache-2.0 — [license](https://github.com/open-telemetry/opentelemetry-go/blob/trace/v1.46.0/trace/LICENSE)
- `go.opentelemetry.io/otel/exporters/prometheus` v0.68.0 — Apache-2.0 — [license](https://github.com/open-telemetry/opentelemetry-go/blob/exporters/prometheus/v0.68.0/exporters/prometheus/LICENSE)
...
**This also raised `github.com/stretchr/testify` to v1.12.1 (from v1.11.1) for
the whole module** — Go's MVS takes the highest version required anywhere in
the graph, and `client_golang`'s own `go.mod` requires a newer floor than we
had. That is **not** a test-only, root-module-only change: `testify` already
reaches `cmd/server` today via `internal/oauth/cache` → `github.com/maypok86/otter/v2`
→ `github.com/stretchr/testify/require` (confirmed with `go mod why -m`), so
this bump is a real release-inventory change, plus a new transitive shipped
dependency (`go.yaml.in/yaml/v3`, pulled in by `testify/assert/yaml`).
Both are handled the normal way — `make refresh-third-party-inventory` (the
`THIRD_PARTY_LICENSES.md` half) plus a `NOTICE` update for `go.yaml.in/yaml/v3`'s
Apache-2.0 §4(d) attribution — not by this section, since they are shipped
components, not test-only ones. See the git history for that commit.
